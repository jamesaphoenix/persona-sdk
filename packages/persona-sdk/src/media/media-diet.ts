...
import { PersonaAttributes } from '../types/index.js';
import { MediaProcessor, MediaContent, UsageMetadata } from './media-processor.js';
...
export interface MediaConsumptionPattern {
  ...
  peakHours: number[];
  avgDailyConsumption: number; // in minutes
  platforms: string[];
  interests: string[];
}
...
export interface MediaDietConfig {
  ...
  personaAttributes?: string[]; // Which attributes to influence
}
...
export class MediaDietManager {
   ...
   */
  async createMediaDiet(
    files: string[],
    defaultPattern?: Partial<MediaConsumptionPattern>
  ): Promise<MediaDietConfig> {
    const items: MediaDietItem[] = [];
    ...
    const patterns: MediaConsumptionPattern = {
   ...
   */
  async applyMediaInfluence(
    persona: Persona,
    diet: MediaDietConfig
  ): Promise<MediaInfluenceResult> {
...
`)
    ...
    const result = await structuredModel.invoke(messages as any);

    // Apply influences to persona attributes
    const influencedAttributes: Record<string, any> = {
      ...persona.attributes
    };
    ...
    personalityAttributes.forEach(trait => {
      const shift = result.personalityShifts[trait as keyof typeof result.personalityShifts];
      if (shift !== 0 && typeof influencedAttributes[trait] === 'number') {
        influencedAttributes[trait] = Math.max(0, Math.min(1, 
          influencedAttributes[trait] + shift * 0.1 // Apply 10% of the shift
        ));
      }
    });
    ...
    const influencedPersona = new Persona(
      `${persona.name} (Media Influenced)`,
      properAttributes as PersonaAttributes
    );
   ...
   */
  async applyToPersonaGroup(
    group: PersonaGroup,
    diet: MediaDietConfig,
    options?: {
      sampleSize?: number;
      variationFactor?: number; // How much personas vary in their response to media
    }
  ): Promise<{
    ...
    const results: MediaInfluenceResult[] = [];
   ...
   */
  private varyMediaDiet(diet: MediaDietConfig, variationFactor: number): MediaDietConfig {
...
`)
    ...
    const recommendations: MediaDietItem[] = [];
    
    const inputTokens = this.mediaProcessor.countTokens(messages.map(m => m.content).join('\n'));
    const outputTokens = this.mediaProcessor.countTokens(response.content as string);
