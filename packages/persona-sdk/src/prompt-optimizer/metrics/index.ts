...
// Re-export types used by metrics
export type { Metric } from '../types/index.js';
 ...
 */
function extractAnswer(value: any): string {
  if (typeof value === 'string') {
    return value;
  }
  if (value && typeof value === 'object') {
    if (value.answer) return String(value.answer);
    if (value.output) return String(value.output);
 ...
 */
export function answerExactMatch(
  example: Example,
 ...
 */
export function answerPassageMatch(
  example: Example,
  prediction: Prediction,
  _trace?: any
): number {
  ...
  } else if (Array.isArray((example as any).context)) {
    context = (example as any).context.map(String);
  } else if (typeof prediction.metadata?.context === 'string') {
    context = [prediction.metadata.context];
  } else if (typeof (example as any).context === 'string') {
    context = [(example as any).context];
  }
 ...
 */
export function answerFuzzyMatch(
  example: Example,
  prediction: Prediction,
  _trace?: any,
 ...
 */
export function answerContainsMatch(
  example: Example,
  prediction: Prediction,
  _trace?: any
): number {
 ...
 */
export function answerNumericMatch(
  example: Example,
  prediction: Prediction,
  _trace?: any,
 ...
 */
export function createCompositeMetric(
  ...
  return {
    ...
    evaluate: (example: Example, prediction: Prediction, _trace?: any): number => {
      const weightedScores = metrics.map(({ metric, weight }) => {
        const score = metric.evaluate(example, prediction, _trace);
        return score * weight;
      });
...
export const PassageMatch: Metric = {
  name: 'passage_match',
...
export const NumericMatch: Metric = {
  name: 'numeric_match',
 ...
 */
export function createExactMatchMetric(frac: number = 1.0): Metric {
  return {
 ...
 */
export function createFuzzyMatchMetric(threshold: number = 0.8): Metric {
  return {
 ...
 */
export function createNumericMatchMetric(tolerance: number = 0.01): Metric {
  return {
