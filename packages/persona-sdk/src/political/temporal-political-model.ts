import { CandidateProfile, PolicyShift, CampaignEvent, PoliticalContext } from './types';
...
export class TemporalPoliticalModel {
  ...
  async updateCandidatePositions(
    ...
    if (updatedCandidate.polling_data) {
      updatedCandidate.polling_data = this.updatePollingFromEvents(
        updatedCandidate.polling_data,
        applicableEvents,
        timePoint
      );
    }
  ...
  calculateMomentum(
    candidate: string,
    recentPolling: Array<{ date: Date; support: number }>,
    recentEvents: CampaignEvent[]
  ): { momentum: number; trend: 'rising' | 'falling' | 'stable'; confidence: number } {
    ...
    if (momentum > 0.02) trend = 'rising';
    else if (momentum < -0.02) trend = 'falling';
   ...
   */
  predictPositionEvolution(
    candidate: CandidateProfile,
    campaignPressures: {
      primary_pressure?: number; // -1 to 1, negative = move left, positive = move right
    ...
    Object.keys(candidate.policy_positions).forEach(issue => {
      ...
      if (salience > 0.5) { // Only predict changes for salient issues
        ...
        if (Math.abs(positionShift) > 0.1) {
          ...
          const predictedShift: PolicyShift = {
  ...
  modelExternalEventImpact(
    ...
    const result = {
      candidate_impacts: {} as Record<string, number>,
      issue_salience_changes: {} as Record<string, number>,
    ...
    switch (event.type) {
      ...
      case 'international_crisis':
        ...
        candidates.forEach(candidate => {
          const hasExperience = candidate.biography.experience.some(exp => 
            exp.includes('foreign') || exp.includes('military') || exp.includes('diplomat')
          );
      ...
      case 'scandal':
        ...
        const targetCandidate = candidates.find(c => 
          event.description.toLowerCase().includes(c.biography.name.toLowerCase())
        );
        if (targetCandidate) {
   ...
   */
  predictLateCampaignDynamics(
    candidates: CandidateProfile[],
    currentDate: Date,
    electionDate: Date,
    currentPolling: Record<string, number>
  ): {
    ...
    const momentumSustainability: Record<string, number> = {};
    candidates.forEach(candidate => {
  ...
  private updatePollingFromEvents(
    polling: CandidateProfile['polling_data'],
  ...
  private calculateTrendConsistency(pollingData: Array<{ date: Date; support: number }>): number {
    if (pollingData.length < 3) return 0.3;
