...
import { NormalDistribution, UniformDistribution, ExponentialDistribution, BetaDistribution, CategoricalDistribution, CorrelatedDistribution, CommonCorrelations } from '../distributions';
/**
...
export class CorrelationAwareSelector {
     ...
     */
    async selectCorrelatedDistributions(params) {
        const tools = [
            {
                type: 'function',
                function: {
...
Ensure the personas will be diverse but realistic.`;
        ...
        for (const dist of result.distributions) {
            distributions[dist.attribute] = this.createDistribution(dist.distribution_type, dist.parameters);
        }
        ...
        for (const cond of result.conditionals) {
            const transform = this.getTransformFunction(cond.relationship_type, cond.custom_formula);
            conditionals.push({
     ...
     */
    async analyzeAndImprove(existingAttributes, context) {
...
4. Missing diversity in categorical attributes`;
    ...
    createDistribution(type, params) {
        switch (type) {
            ...
            case 'categorical':
                return new CategoricalDistribution(params.categories.map((c) => ({
                    value: c.value,
     ...
     */
    getTransformFunction(type, customFormula) {
        switch (type) {
            case 'age_experience':
            ...
            case 'location_income':
                return (income, location) => {
                    ...
                    return income * (multipliers[location] || 1);
                };
            case 'custom':
                // Parse and create custom function
                // This is simplified - in production, use a safe expression parser
                return new Function('value', 'dependent', customFormula || 'return value');
            default:
                return (value) => value;
        }
    }
 ...
 */
export async function generateRealisticPersonas(_count, context, options) {
...
//# sourceMappingURL=correlation-aware-selector.js.map