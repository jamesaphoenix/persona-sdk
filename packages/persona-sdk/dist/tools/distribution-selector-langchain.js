...
Select the best distribution type and parameters.`;
        ...
        switch (result.distribution_type) {
            case 'normal':
                distribution = new NormalDistribution(result.parameters.mean || 0, result.parameters.std_dev || 1);
                break;
            case 'uniform':
                distribution = new UniformDistribution(result.parameters.min || 0, result.parameters.max || 1);
                break;
            case 'exponential':
                distribution = new ExponentialDistribution(result.parameters.rate || 1);
                break;
            case 'beta':
                distribution = new BetaDistribution(result.parameters.alpha || 2, result.parameters.beta || 2);
                break;
            case 'categorical':
                const categories = result.parameters.categories;
                distribution = new CategoricalDistribution((!categories || categories.length === 0) ? [
...
Consider relationships between attributes (e.g., age affects income).`;
        ...
        result.recommendations.forEach(rec => {
            ...
            switch (rec.distribution_type) {
                case 'normal':
                    dist = new NormalDistribution(rec.parameters.mean || 0, rec.parameters.std_dev || 1);
                    break;
                case 'uniform':
                    dist = new UniformDistribution(rec.parameters.min || 0, rec.parameters.max || 1);
                    break;
                case 'exponential':
                    dist = new ExponentialDistribution(rec.parameters.rate || 1);
                    break;
                case 'beta':
                    dist = new BetaDistribution(rec.parameters.alpha || 2, rec.parameters.beta || 2);
                    break;
                case 'categorical':
                    dist = new CategoricalDistribution(rec.parameters.categories || []);
                    break;
                default:
...
- "job titles" → Categorical with appropriate probabilities`;
        ...
        switch (result.distribution_type) {
            ...
            case 'beta':
                ...
                break;
            case 'categorical':
                distribution = new CategoricalDistribution(result.parameters.categories || []);
                break;
            default:
                distribution = new NormalDistribution(50, 10);
        }
     ...
     */
    estimateCost(attributeCount, model = 'gpt-4-turbo-preview') {
...
//# sourceMappingURL=distribution-selector-langchain.js.map