...
export class BootstrapOptimizer extends BaseOptimizer {
    ...
    async optimize(student, trainset, valset = []) {
        ...
        let bootstrappedExamples = [];
        if (this.bootstrapConfig.teacherModel && this.bootstrapConfig.maxBootstrapped > 0) {
            bootstrappedExamples = await this.generateBootstrappedExamples(student, trainset, this.bootstrapConfig.teacherModel);
            if (this.bootstrapConfig.verbose) {
        ...
        const finalEvaluation = await this.evaluate(optimizedModule, evaluationSet, this.bootstrapConfig.metric);
        const round = {
            round: 1,
     ...
     */
    selectLabeledExamples(trainset) {
     ...
     */
    async generateBootstrappedExamples(student, trainset, teacher) {
        ...
        while (bootstrappedExamples.length < this.bootstrapConfig.maxBootstrapped && attempts < maxAttempts) {
            ...
            try {
                ...
                const score = this.bootstrapConfig.metric.evaluate(bootstrappedExample, studentPrediction);
...
Provide only the output, without any additional explanation:`;
     ...
     */
    async createOptimizedModule(student, demonstrations) {
...
Now, please respond to the following input:`;
     ...
     */
    createDemonstrationPrompt(examples) {
