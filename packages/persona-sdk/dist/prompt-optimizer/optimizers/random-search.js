...
export class RandomSearchOptimizer extends BaseOptimizer {
            ...
            try {
                ...
                history.push({
                    ...
                    metadata: {
                        ...
                        demonstrationCount: candidate._demonstrations.length,
                        ...candidate.metadata,
                    },
                });
    ...
    async generateMutationCandidate(module, trainset, index) {
        ...
        const _demonstrations = this.sampleDemonstrations(trainset);
        let mutatedPrompt = basePrompt;
        if (this.llm) {
            mutatedPrompt = await this.mutatePrompt(basePrompt, _demonstrations);
        }
        return {
            ...
            metadata: {
                ...
                demonstrationCount: _demonstrations.length,
    ...
    async generateCrossoverCandidate(module, trainset, index) {
        ...
        const crossoverPrompt = this.combinePromptElements(basePrompt, _demonstrations);
        return {
    ...
    async generateRandomCandidate(module, trainset, index) {
        ...
        const randomPrompt = this.randomizePromptStructure(basePrompt, _demonstrations);
        return {
            prompt: randomPrompt,
            score: 0,
            _demonstrations,
    ...
    /**
     * Sample random _demonstrations from the training set
     */
    sampleDemonstrations(trainset) {
        const maxDemos = Math.min(8, trainset.length); // Cap at 8 _demonstrations
     ...
     */
    async mutatePrompt(basePrompt, _demonstrations) {
        if (!this.llm) {
            return basePrompt;
        }
...
Mutated prompt:`;
     ...
     */
    combinePromptElements(basePrompt, _demonstrations) {
        // Simple crossover: modify the demonstration section
        const demonstrationText = this.formatDemonstrations(_demonstrations);
        return `${basePrompt}
...
Please provide your response:`;
     ...
     */
    randomizePromptStructure(basePrompt, _demonstrations) {
        const structures = [
            // Structure 1: Examples first
            () => {
                const demos = this.formatDemonstrations(_demonstrations);
    ...
    /**
     * Format _demonstrations for inclusion in prompts
     */
    formatDemonstrations(_demonstrations) {
        return _demonstrations.map((demo, index) => {
