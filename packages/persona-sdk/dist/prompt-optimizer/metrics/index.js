 ...
 */
export function answerExactMatch(example, prediction, _trace, _frac = 1.0) {
 ...
 */
export function answerPassageMatch(example, prediction, _trace) {
    const predictedAnswer = extractAnswer(prediction.output);
    const normalizedPredicted = normalizeText(predictedAnswer);
 ...
 */
export function answerFuzzyMatch(example, prediction, _trace, threshold = 0.8) {
    const expectedAnswer = extractAnswer(example.output);
    const predictedAnswer = extractAnswer(prediction.output);
    const normalizedExpected = normalizeText(expectedAnswer);
    const normalizedPredicted = normalizeText(predictedAnswer);
 ...
 */
export function answerContainsMatch(example, prediction, _trace) {
    const expectedAnswer = extractAnswer(example.output);
    const predictedAnswer = extractAnswer(prediction.output);
    const normalizedExpected = normalizeText(expectedAnswer);
    const normalizedPredicted = normalizeText(predictedAnswer);
 ...
 */
export function answerNumericMatch(example, prediction, _trace, tolerance = 0.01) {
    const expectedAnswer = extractAnswer(example.output);
    const predictedAnswer = extractAnswer(prediction.output);
...
export function createCompositeMetric(metrics) {
    ...
    return {
        ...
        evaluate: (example, prediction, _trace) => {
