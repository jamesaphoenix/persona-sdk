...
export class PoliticalAnalysisFramework {
    ...
    config;
    constructor(_apiKey, voterPersonas, config = {}) {
        this._apiKey = _apiKey;
        this.voterPersonas = voterPersonas;
     ...
     */
    async predictElectionOutcome(election) {
     ...
     */
    async analyzeDemographics(election) {
        ...
        const result = await this.structuredOutputGenerator.generateCustom(this.voterPersonas, DemographicAnalysisSchema, prompt);
        return result.data;
    }
    /**
     * Analyze ideological alignment between voters and candidates
     */
    async analyzeIdeology(election) {
        ...
        const result = await this.structuredOutputGenerator.generateCustom(this.voterPersonas, IdeologicalAnalysisSchema, prompt);
        return result.data;
    }
    /**
     * Analyze temporal dynamics affecting the election
     */
    async analyzeTemporalDynamics(election) {
        ...
        const result = await this.structuredOutputGenerator.generateCustom(this.voterPersonas, TemporalAnalysisSchema, prompt);
        return result.data;
    }
    /**
    ...
    async chainOfThoughtPrediction(analysis) {
        ...
        const result = await this.structuredOutputGenerator.generateCustom(analysis.personas, PredictionSchema, prompt);
     ...
     */
    async simulateVoterResponse(voters, candidates, context) {
        ...
        const result = await this.structuredOutputGenerator.generateCustom(voters, ResponseSimulationSchema, prompt);
        return result.data;
    }
    // Private methods for building prompts
    buildDemographicAnalysisPrompt(election) {
...
Focus on segments that could be decisive in this election. Consider how demographic trends and generational changes might affect traditional voting patterns.`;
    ...
    buildIdeologicalAnalysisPrompt(election) {
...
Consider how issue salience affects the weight of different ideological positions.`;
    ...
    buildTemporalAnalysisPrompt(election) {
...
Consider how the ${this.config.temporal_window_days}-day window leading up to the election might affect outcomes.`;
    ...
    buildChainOfThoughtPrompt(analysis) {
...
Confidence level should reflect the degree of uncertainty in the prediction. Lower confidence for close races or when conflicting signals exist.`;
    ...
    buildVoterResponsePrompt(candidates, context) {
...
Provide demographic breakdowns and identify the most volatile voter segments.`;
    ...
    // Helper methods
    calculateElectoralVotes(stateResults) {
    ...
    extractMomentum(temporalAnalysis) {
        const momentum = {};
        if (temporalAnalysis.momentum_analysis) {
            Object.entries(temporalAnalysis.momentum_analysis).forEach(([candidate, data]) => {
                momentum[candidate] = data.current_momentum || 0;
            });
        }
        return momentum;
    }
    calculateExternalEventImpact(temporalAnalysis) {
        if (!temporalAnalysis.event_impacts)
            return 0;
        return temporalAnalysis.event_impacts.reduce((total, event) => {
            return total + Math.abs(event.impact_magnitude || 0);
        }, 0) / temporalAnalysis.event_impacts.length;
    }
}
//# sourceMappingURL=political-analysis-framework.js.map