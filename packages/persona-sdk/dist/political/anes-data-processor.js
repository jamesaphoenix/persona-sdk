...
export class ANESDataProcessor {
     ...
     */
    async loadANESData(year, options = { year }) {
     ...
     */
    async generateSyntheticVoters(baseData, targetCount) {
     ...
     */
    async convertANESToPersonas(data) {
     ...
     */
    generateSyntheticANESRecord(year, index) {
        // Use deterministic randomness based on index for reproducibility
        const random = (seed) => Math.sin(seed * 9999) * 0.5 + 0.5;
     ...
     * Generate correlated ideology based on demographics
     */
    generateIdeology(age, education, race, party, rand) {
     ...
     */
    generateIssuePositions(baseScale, _party) {
        const issues = {
    ...
    calculateEducationDistribution(data) {
        const counts = {};
        data.forEach(v => {
    ...
    calculateIncomeDistribution(data) {
    ...
    calculateRaceDistribution(data) {
        const counts = {};
        data.forEach(v => {
    ...
    calculateGeographyDistribution(data) {
        const counts = {};
        data.forEach(v => {
    ...
    calculatePartyDistribution(data) {
        const counts = {};
        data.forEach(v => {
    ...
    calculateIdeologyDistribution(data) {
        const counts = {};
        data.forEach(v => {
    ...
    calculateIssueCorrelations(data) {
        ...
        issues.forEach(issue1 => {
            ...
            issues.forEach(issue2 => {
                if (issue1 !== issue2) {
                    correlations[issue1][issue2] = this.calculateCorrelation(data.map(v => v.ideology.issue_positions[issue1] || 0), data.map(v => v.ideology.issue_positions[issue2] || 0));
                }
            });
        });
        return correlations;
    }
    calculateTurnoutPatterns(data) {
    ...
    calculateVoteMethodDistribution(data) {
        const counts = {};
        data.forEach(v => {
    ...
    passesFilters(voter, filters) {
        if (!filters)
            return true;
        if (filters.min_age && voter.demographics.age < filters.min_age)
            return false;
        if (filters.max_age && voter.demographics.age > filters.max_age)
            return false;
        if (filters.states && !filters.states.includes(voter.demographics.state || ''))
            return false;
        if (filters.education_levels && !filters.education_levels.includes(voter.demographics.education))
            return false;
        return true;
    }
    generateIncome(age, education, rand) {
        let baseIncome = 35000; // Base income
        // Age influence (experience)
        if (age > 30)
            baseIncome += (age - 30) * 1000;
        if (age > 50)
            baseIncome += (age - 50) * 500;
    ...
    generateVotingHistory(age, currentYear, rand) {
        const history = [];
    ...
    generateTurnoutLikelihood(age, education, party, rand) {
        ...
        if (education === 'Graduate')
            likelihood += 0.2;
        else if (education === 'College Grad')
            likelihood += 0.1;
        else if (education === 'Less than HS')
            likelihood -= 0.15;
    ...
    generateNewsSources(party, age, rand) {
        const sources = [];
    ...
    generateSyntheticVoter(_patterns, index) {
...
//# sourceMappingURL=anes-data-processor.js.map