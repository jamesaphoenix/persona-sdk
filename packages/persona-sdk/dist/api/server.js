...
import { createPersonaSchema, updatePersonaSchema, personaQuerySchema, createPersonaGroupSchema, updatePersonaGroupSchema, personaGroupQuerySchema, bulkCreatePersonasSchema, addToGroupSchema, removeFromGroupSchema, personaResponseSchema, personaGroupResponseSchema, paginatedResponseSchema, statsResponseSchema, errorResponseSchema, } from './schemas.js';
export async function createServer(options) {
    ...
    app.post('/personas', {
        ...
        const persona = await adapter.createPersona(request.body);
    ...
    app.get('/personas/:id', {
        ...
        const persona = await adapter.getPersona(request.params.id);
        if (!persona) {
    ...
    app.patch('/personas/:id', {
        ...
        const persona = await adapter.updatePersona(request.params.id, request.body);
        if (!persona) {
            reply.code(404).send({ error: 'Persona not found' });
            return;
        }
    ...
    app.delete('/personas/:id', {
        ...
        const deleted = await adapter.deletePersona(request.params.id);
        if (!deleted) {
            reply.code(404).send({ error: 'Persona not found' });
            return;
        }
    ...
    app.post('/personas/bulk', {
        ...
        const personas = await adapter.bulkCreatePersonas(request.body);
    ...
    app.post('/groups', {
        ...
        const group = await adapter.createPersonaGroup(request.body);
    ...
    app.get('/groups/:id', {
        ...
        const group = await adapter.getPersonaGroup(request.params.id);
        if (!group) {
    ...
    app.patch('/groups/:id', {
        ...
        const group = await adapter.updatePersonaGroup(request.params.id, request.body);
        if (!group) {
            reply.code(404).send({ error: 'Group not found' });
            return;
        }
    ...
    app.delete('/groups/:id', {
        ...
        const deleted = await adapter.deletePersonaGroup(request.params.id);
        if (!deleted) {
            reply.code(404).send({ error: 'Group not found' });
            return;
        }
    ...
    app.get('/groups/:id/members', {
        ...
        const result = await adapter.getPersonaGroupWithMembers(request.params.id);
        if (!result) {
            reply.code(404).send({ error: 'Group not found' });
            return;
        }
    ...
    app.post('/memberships', {
        ...
        const success = await adapter.addPersonaToGroup(request.body.personaId, request.body.groupId);
    ...
    app.delete('/memberships', {
        ...
        const success = await adapter.removePersonaFromGroup(request.body.personaId, request.body.groupId);
        if (!success) {
    ...
    app.get('/personas/:id/groups', {
        ...
        return adapter.getPersonaGroups(request.params.id);
    });
    // ============ Stats Route ============
    app.get('/stats', {
        schema: {
